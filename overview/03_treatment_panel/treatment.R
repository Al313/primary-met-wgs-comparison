...
sample_metadata[,"cancer_type_code"] <- factor(sample_metadata[,"cancer_type_code"],
                                               levels = cancer_type_code_order_fig1)

sample_metadata[,"gender"] <- factor(sample_metadata[,"gender"], levels = c("MALE", "FEMALE"))

sample_metadata_hmf <- metadata_included[metadata_included$cohort == "Hartwig",]
...
agg_treatment_data <- agg_treatment_data[agg_treatment_data$sample_id %in% metadata_included$sample_id,]
...
## Making the summary plot using ggplot2
